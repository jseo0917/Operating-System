/*
    ...
    Author: R. Bettati, changed by D. Da Silva
...
#define PROCESS_POOL_START_FRAME ((4 MB) / (4 KB))  // 1KB
#define PROCESS_POOL_SIZE ((28 MB) / (4 KB)) // 7KB
...
int main() {
    ...
    //debug_out_E9_msg_value("n_info_frames", n_info_frames);
    
    unsigned long process_mem_pool_info_frame = kernel_mem_pool.get_frames(n_info_frames);
   
    //debug_out_E9_msg_value("n_info_frames", kernel_mem_pool.get_frames(n_info_frames));
    //Console::puts(process_mem_pool_info_frame);
    //debug_out_E9_msg_value("Process_mem_pool_info_frame", process_mem_pool_info_frame);
    ...
    // TEST BOTH KERNEL AND PROCESS POOL
