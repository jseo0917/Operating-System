...
int main(int argc, char** argv) {
    ...
    /***********************************************************
     *  NEW TESTS - not in gtest.C 
     ***********************************************************/

    // allocate another pool with larger area to check it is not writing
    // beyond the allocated area
    
    // Testing with a pool for frames 2048 to 4095
    const unsigned long POOL_NB_FRAMES_2 = 2048;
    const unsigned long BASE_FRAME_2 = 2048;
    unsigned int bytes_needed_2 = 
               POOL_NB_FRAMES_2 / (8 / Manager::NumberBitsRepresentingFrame());
    char* area_2 = new char[bytes_needed_2+1];
    Manager frame_pool_2 = Manager((unsigned long long) area_2,
                                 POOL_NB_FRAMES_2, BASE_FRAME_2);
    // special mark on area out of the array
    area_2[bytes_needed_2] = '*';

    EXPECT_EQ (frame_pool.FREE, frame_pool_2.get_frame_state(BASE_FRAME_2+100));
    EXPECT_EQ (frame_pool.FREE, frame_pool_2.get_frame_state(
                                          BASE_FRAME_2+POOL_NB_FRAMES_2-1));

    unsigned long frame2048_2 = frame_pool_2.get_frames(2048);
    EXPECT_EQ('*', area_2[bytes_needed_2]);
    EXPECT_EQ(true, frame_pool_2.release_frames(frame2048_2));
    EXPECT_EQ('*', area_2[bytes_needed_2]);
    EXPECT_EQ(frame_pool_2.FREE, area_2[bytes_needed_2-1]);         
    cout << "Tests for frame_pool_2 passed." << endl;
